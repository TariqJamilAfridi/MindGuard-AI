/* ═══════════════════════════════════════════════════════════
   MINDGUARD — script.js  v4
   + Enter key to submit
   + Text-to-Speech for responses (Web Speech Synthesis API)
   + All mood animations, voice input, auth, charts
═══════════════════════════════════════════════════════════ */
...
function spawnHappy() {
  ...
  for (let i = 0; i < 160; i++) {
    ...
    particles.push({ type:'confetti', x:canvas.width*(.2+Math.random()*.6), y:canvas.height*(.3+Math.random()*.4),
      vx:Math.cos(a)*s, vy:Math.sin(a)*s-3, rot:Math.random()*360, rotV:(Math.random()-.5)*8,
      w:6+Math.random()*9, h:3+Math.random()*5, color:colors[Math.floor(Math.random()*colors.length)], alpha:1 });
  }
  for (let i = 0; i < 40; i++) {
    particles.push({ type:'star', x:Math.random()*canvas.width, y:canvas.height+30,
      vy:-(0.5+Math.random()*1.5), size:4+Math.random()*12, alpha:0, pulse:Math.random()*Math.PI*2 });
  }
}
...
function spawnSad() {
  particles = [];
  for (let i = 0; i < 120; i++) {
    particles.push({ type:'rain', x:Math.random()*canvas.width, y:-Math.random()*canvas.height,
      vy:4+Math.random()*6, vx:-1+Math.random()*.5, len:14+Math.random()*22, alpha:.1+Math.random()*.35 });
  }
  for (let i = 0; i < 6; i++) {
    particles.push({ type:'ripple', x:canvas.width*(.1+Math.random()*.8), y:canvas.height*(.6+Math.random()*.35),
      r:0, maxR:80+Math.random()*120, alpha:.4, speed:0.7+Math.random()*.5 });
  }
}
...
function spawnAnxious() {
  ...
  for (let i=0;i<12;i++) particles.push({ type:'pulse', x:cx, y:cy, r:i*30,
    maxR:Math.max(canvas.width,canvas.height)*.8, alpha:.6, speed:3+i*.4, color:i%2===0?'#c97a62':'#c9a96e' });
  for (let i=0;i<50;i++) {
    const a=Math.random()*Math.PI*2;
    particles.push({ type:'spark', x:cx+Math.cos(a)*(40+Math.random()*80), y:cy+Math.sin(a)*(40+Math.random()*80),
      vx:(Math.random()-.5)*4, vy:(Math.random()-.5)*4, alpha:.8, size:1+Math.random()*2 });
  }
}
...
function spawnExhausted() {
  particles = [];
  for (let i=0;i<60;i++) particles.push({ type:'droop', x:Math.random()*canvas.width,
    y:-20-Math.random()*canvas.height*.5, vy:0.2+Math.random()*.5, vx:(Math.random()-.5)*.3,
    r:3+Math.random()*9, alpha:.08+Math.random()*.18, color:Math.random()>.5?'#5a5550':'#1e252c',
...
function drawHappy() {
  particles.forEach(p => {
    if (p.type==='confetti') {
      p.vy+=.12; p.x+=p.vx; p.y+=p.vy; p.rot+=p.rotV; p.alpha-=.004;
      if (p.alpha<=0) resetConfetti(p);
      ctx2d.save(); ctx2d.globalAlpha=Math.max(0,p.alpha);
    ...
    if (p.type==='star') {
      p.y+=p.vy; p.pulse+=.04; p.alpha=Math.min(.7,p.alpha+.01)*(0.6+0.4*Math.sin(p.pulse));
      if (p.y<-30) resetStar(p);
      ctx2d.save(); ctx2d.globalAlpha=p.alpha; ctx2d.fillStyle='#e8c98a';
      ctx2d.font=`${p.size}px serif`; ctx2d.fillText('✦',p.x,p.y); ctx2d.restore();
    }
  });
}
function resetConfetti(p) {
  const c=['#c9a96e','#82b09a','#e8c98a','#c97a62','#fff9e6'];
  p.x=canvas.width*(.1+Math.random()*.8); p.y=canvas.height*(.2+Math.random()*.3);
  const a=Math.random()*Math.PI*2,s=2+Math.random()*5; p.vx=Math.cos(a)*s; p.vy=Math.sin(a)*s-2;
  p.alpha=.9; p.color=c[Math.floor(Math.random()*c.length)];
}
function resetStar(p) { p.x=Math.random()*canvas.width; p.y=canvas.height+20; p.alpha=0; }
...
function drawSad() {
  particles.forEach(p => {
    if (p.type==='rain') {
      ...
      ctx2d.save(); ctx2d.globalAlpha=p.alpha; ctx2d.strokeStyle='#4a7a9a'; ctx2d.lineWidth=1.2;
      ctx2d.beginPath(); ctx2d.moveTo(p.x,p.y); ctx2d.lineTo(p.x+p.vx*2,p.y+p.len); ctx2d.stroke(); ctx2d.restore();
    }
    if (p.type==='ripple') {
      p.r+=p.speed; p.alpha-=.004;
      if (p.alpha<=0) { p.r=0; p.alpha=.4; p.x=canvas.width*(.1+Math.random()*.8); }
      ctx2d.save(); ctx2d.globalAlpha=p.alpha; ctx2d.strokeStyle='#3a5a7a'; ctx2d.lineWidth=1;
      ctx2d.beginPath(); ctx2d.arc(p.x,p.y,p.r,0,Math.PI*2); ctx2d.stroke(); ctx2d.restore();
    }
  });
}
...
function drawAnxious() {
  particles.forEach(p => {
    if (p.type==='pulse') {
      p.r+=p.speed; p.alpha-=.006; if (p.r>p.maxR) { p.r=0; p.alpha=.5; }
      ctx2d.save(); ctx2d.globalAlpha=Math.max(0,p.alpha); ctx2d.strokeStyle=p.color; ctx2d.lineWidth=1.5;
      ctx2d.beginPath(); ctx2d.arc(p.x,p.y,p.r,0,Math.PI*2); ctx2d.stroke(); ctx2d.restore();
    }
    if (p.type==='spark') {
      p.x+=p.vx+(Math.random()-.5)*.5; p.y+=p.vy+(Math.random()-.5)*.5; p.alpha=.5+Math.random()*.5;
      ctx2d.save(); ctx2d.globalAlpha=p.alpha; ctx2d.fillStyle='#c97a62';
...
function drawExhausted() {
  particles.forEach(p => {
    p.wobble+=.02; p.x+=p.vx+Math.sin(p.wobble)*.3; p.y+=p.vy;
...
function triggerAnimation(sentiment, emotions) {
  ...
  if (sentiment==='Positive'||['joy','pride','hope','gratitude'].includes(primary)) {
    animMode='happy'; spawnHappy();
  } else if (['anxiety','anxious','confusion','anger'].includes(primary)) {
    animMode='anxious'; spawnAnxious();
  } else if (['exhaustion','loneliness'].includes(primary)) {
    animMode='exhausted'; spawnExhausted();
  } else if (sentiment==='Negative') {
    animMode='sad'; spawnSad();
  } else {
    animMode=null; canvas.classList.remove('visible'); return;
  }
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      canvas.classList.remove('visible'); canvas.style.opacity=''; canvas.style.transition='';
...
function speakResponse(text) {
  ...
  utterance.pitch  = 1.05;    // slightly higher = friendlier
...
function toggleSpeak() {
  if (isSpeaking) {
    ...
    const responseEl = document.getElementById('responseQuote');
    const tipEl      = document.getElementById('tipBlock');
    if (!responseEl) return;
    // Read both response and tip
    const responseText = responseEl.textContent.trim();
    const tipText      = tipEl ? tipEl.textContent.replace('💡 Try this:', 'Here is a tip for you.') : '';
    speakResponse(responseText + ' ... ' + tipText);
  }
}
...
function updateSpeakBtn(speaking) {
  ...
  btn.innerHTML = speaking
    ? `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Stop`
    : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/></svg> Read Aloud`;
...
function setGreeting() {
  ...
  document.getElementById('greetingText').textContent =
    h<5?'Up late, are we?':h<12?'Good morning':h<17?'Good afternoon':h<21?'Good evening':'Good night';
...
function setupTextarea() {
  ...
  // ── ENTER KEY TO SUBMIT ──────────────────────────────────
  // Shift+Enter = new line (normal)
  // Enter alone = submit entry
...
function toggleSidebar() {
  const sb=document.getElementById('sidebar'), ov=document.getElementById('sidebarOverlay'), hb=document.getElementById('hamburger');
  const open=sb.classList.toggle('open');
  ov.classList.toggle('visible',open); hb.classList.toggle('open',open);
}
...
async function submitEntry() {
  ...
  try {
    ...
    // Auto-speak the response after a short delay
    setTimeout(() => speakResponse(data.response), 800);

    if (data.show_nudge && !currentUser) setTimeout(showNudge, 1500);
...
function renderResult(data) {
  ...
  pill.textContent=data.sentiment; pill.className=`sentiment-pill pill-${data.sentiment}`;

  const pct=((data.score+1)/2)*100;
  const bar=document.getElementById('scoreBar');
  bar.style.width=`${pct}%`; bar.style.background=scoreColor(data.sentiment);
...
function setOrb(sentiment) {
  ...
  const m={Positive:['✦','Positive'],Neutral:['◉','Balanced'],Negative:['◌','Low']};
  orb.className=`orb ${sentiment.toLowerCase()}`;
  [emoji.textContent,label.textContent]=m[sentiment]||['✦','—'];
}
...
function dismissNudge() { document.getElementById('nudgeBanner').style.display='none'; }
...
async function doRegister() {
  ...
  try {
    ...
    showToast(`Welcome, ${data.user.name}! 🎉`); allEntries=[];
  } catch(e) { errEl.textContent='Something went wrong.'; }
}
...
async function doLogin() {
  ...
  try {
    ...
    if (!res.ok) { errEl.textContent=data.error; return; }
    setCurrentUser(data.user); closeModal();
    showToast(`Welcome back, ${data.user.name}! ✦`); allEntries=[];
  } catch(e) { errEl.textContent='Something went wrong.'; }
}
...
async function doLogout() {
  ...
  allEntries=[]; showToast('Signed out. See you soon 👋');
}
...
function showToast(msg) {
  const ex=document.getElementById('mg-toast'); if(ex) ex.remove();
  const t=document.createElement('div'); t.id='mg-toast'; t.textContent=msg;
...
async function initDashboard() {
  await Promise.all([loadMoodChart(7,document.querySelector('.rtbtn.active')),loadEmotionChart(),loadSentimentChart(),loadStats(),checkTrend(),generateInsight()]);
}
...
async function loadMoodChart(days,btn) {
  ...
  chartObjs.mood=new Chart(document.getElementById('moodChart').getContext('2d'),{
    type:'bar', data:{labels,datasets:[{data:scores,backgroundColor:colors,borderRadius:6,borderWidth:0}]},
    options:{responsive:true,plugins:{tooltip:{callbacks:{label:c=>` Score: ${c.parsed.y>0?'+':''}${c.parsed.y}`}}},
      scales:{y:{min:-1,max:1,grid:{color:'rgba(255,255,255,.04)'},ticks:{callback:v=>v===0?'0':v>0?`+${v}`:v,stepSize:.5}},x:{grid:{display:false}}}}
  });
}

async function loadEmotionChart() {
  const counts=await fetch('/api/emotions').then(r=>r.json());
  const labels=Object.keys(counts).slice(0,7), values=Object.values(counts).slice(0,7);
  ...
  chartObjs.emotion=new Chart(document.getElementById('emotionChart').getContext('2d'),{
    type:'doughnut', data:{labels,datasets:[{data:values,backgroundColor:palette,borderWidth:0,hoverOffset:8}]},
...
async function loadSentimentChart() {
  ...
  chartObjs.sentiment=new Chart(document.getElementById('sentimentChart').getContext('2d'),{
    type:'pie', data:{labels:['Positive','Neutral','Negative'],datasets:[{data:[pos,neu,neg],backgroundColor:['rgba(130,176,154,.8)','rgba(201,169,110,.8)','rgba(201,122,98,.8)'],borderWidth:0,hoverOffset:8}]},
...
async function generateInsight() {
  ...
  const counts={Positive:0,Neutral:0,Negative:0}, ef={};
  entries.forEach(e=>{
    counts[e.sentiment]++;
  ...
  const tail=dom==='Positive'?"You have been carrying real brightness lately. Keep going. ✨"
            :dom==='Negative'?"You have been going through a lot. Reaching out for support is always okay."
            :"Your mood has been steady. That kind of consistency is quietly powerful.";
...
function renderHistory(entries) {
  ...
  if(!entries.length){
    list.innerHTML='<div class="empty-state"><div class="empty-icon">📖</div><p>No entries yet. Write your first one!</p></div>'; return;
  }
...
async function exportCSV() {
  ...
  const csv=[['Date','Sentiment','Score','Emotions','Text'],...allEntries.map(e=>[e.created_at,e.sentiment,e.score,`"${e.emotions}"`,`"${e.text.replace(/"/g,'""')}"`])].map(r=>r.join(',')).join('\n');
  const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(new Blob([csv],{type:'text/csv'})),download:`mindguard-${new Date().toISOString().slice(0,10)}.csv`});
  a.click();
}
...
function updateStreak() {
  const today=new Date().toISOString().slice(0,10),last=localStorage.getItem('mg_last')||'',yest=new Date(Date.now()-86400000).toISOString().slice(0,10);
...
function loadStreak(){document.getElementById('streakCount').textContent=localStorage.getItem('mg_streak')||'0';}


// ════════════════════════════════════════════════════════════
//  HELPERS
// ════════════════════════════════════════════════════════════
function scoreColor(s){return s==='Positive'?'#82b09a':s==='Negative'?'#c97a62':'#c9a96e';}
function emEmoji(e){return{Joy:'😊',Sadness:'😢',Anger:'😠',Anxiety:'😰',Exhaustion:'😴',Confusion:'😕',Hope:'🌱',Gratitude:'🙏',Loneliness:'🫂',Pride:'🌟',Neutral:'😐',Calm:'🌊',Unease:'😶'}[e]||'💭';}
